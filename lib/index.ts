/**
 * First-order function that rearranges an indexed array of objects by the specified key, based on a key of the object.
 * For use within `Array.sort()`
 *
 ...
 */
function compareObjectsByKey<T = Record<any, any>>(
  key: keyof T,
  ascending = true
) {
...
/**
 * Sort the elements of an array alphanumerically by the specified key in ascending (default) order.
 * @param {T[]} array
 * @param {keyof T} key
 * @param {boolean} ascending
 * @returns {T[]} sorted array
 */
export function sortArrayByKey<T = Record<any, any>>(
  array: T[],
  key: keyof T,
  ascending = true
) {
  return array.sort(compareObjectsByKey(key, ascending));
}
